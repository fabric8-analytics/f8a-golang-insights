...
def parse_glide_yaml(mercator_op):
...
def parse_gopkg_toml(mercator_op):
...
manifests_glide = []
with open(
        '/Users/avgupta/f8a/mercator-go/manifests_kube/schemakube_ecosystem_glide_yaml.json') as f:
    content = f.read()
...
manifests_dep = []
with open(
        '/Users/avgupta/f8a/mercator-go/manifests_kube/schemakube_ecosystem_gopkg_toml.json') as f:
    content = f.read()
json_content = json.loads(content)
if not os.path.exists('/tmp/current_workdir'):
    os.makedirs('/tmp/current_workdir')
...
manifests_godeps = []
with open('/Users/avgupta/f8a/mercator-go/manifests_kube/schemakube_ecosystem_godeps.json') as f:
    content = f.read()
json_content = json.loads(content)
if not os.path.exists('/tmp/current_workdir'):
    os.makedirs('/tmp/current_workdir')
